...
def main(args=None):
    ...
    # 시작 좌표
    place_start_pose = posx([561.52, 221.63, 85.13, 0.0, 180.0, 0.0]) # 221.63  
    fourth_floor_start_pose = posx([561.52, 221.63, 306.26, 95.25, 90.0, -90.0])

    # 이동 좌표
    first_floor_list = [(0, 0), (1.0, 0), (2.0, 0), (0.5, 1.0), (1.5, 1.0), (1.0, 2.0)]
    second_floor_list = [(0.5, 0.5), (1.5, 0.5), (1.0, 1.5)]    
    third_floor_list = (1.0,1.0)
    fourth_floor_list = (1.0,0.6)

    frist_floor_z = 85.13
    second_floor_z = 178.1
    third_floor_z = 272.07
    ...
    def first_floor(start_pose,pos_x):
        global spline_list

        for xy in first_floor_list:
            world_cup()        
    ...
    def second_floor(start_pose, pos_x):
        global spline_list

        for xy in second_floor_list:
            world_cup()  
    ...
    def third_floor(start_pose, pos_x):
        global spline_list         
        ...
        pose[0] -= CUP_DIAMETER_X * third_floor_list[0]

        pose[1] -= CUP_DIAMETER_Y * third_floor_list[1]
    ...
    first_floor(place_start_pose,frist_floor_z)
    second_floor(place_start_pose,second_floor_z)
    third_floor(place_start_pose, third_floor_z)
